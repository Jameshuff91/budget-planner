...
export default function RecurringTransactionsView() {
  ...
  const isDataAvailable =
    potentialRecurringTransactions && potentialRecurringTransactions.length > 0;
    ...
    <Card>
      ...
      <CardContent>
        <div className='flex space-x-2 mb-4'>
          ...
          >
            All ({potentialRecurringTransactions?.length || 0})
          </Button>
          <Button
          ...
          >
            Pending (
            {potentialRecurringTransactions?.filter((c) => !recurringPreferences[c.id]).length || 0}
            )
          </Button>
          <Button
        ...
        {!isDataAvailable ? (
          ...
          <ScrollArea className='h-[350px] pr-3'>
            {' '}
            {/* Adjusted height slightly for filter buttons */}
            <div className='space-y-3'>
              {filteredCandidates.map((candidate) => {
                const currentStatus = recurringPreferences[candidate.id]; // Use context's recurringPreferences
                let itemClasses =
                  'py-3 px-2 border-b last:border-b-0 hover:bg-gray-50 rounded-md transition-all duration-150 ease-in-out';
                ...
                if (currentStatus === 'confirmed') {
                  ...
                  statusBadge = (
                    <span className='ml-2 text-xs text-green-700 inline-flex items-center'>
                  ...
                  statusBadge = (
                    <span className='ml-2 text-xs text-gray-500 inline-flex items-center'>
                      <XCircle size={14} className='mr-1' /> Dismissed
                    </span>
                  );
                }
                ...
                return (
                  <div key={candidate.id} className={itemClasses}>
                    ...
                    <p
                      className={`text-sm text-gray-600 ${currentStatus === 'dismissed' ? 'line-through' : ''}`}
                    >
                      Frequency:{' '}
                      <span
                        className={`font-medium ${currentStatus === 'dismissed' ? 'line-through' : ''}`}
                      >
                        {capitalizeFirstLetter(candidate.frequency)}
                      </span>
                    </p>
                    ...
                    <p className='text-xs text-gray-400 mt-1'>
                      (Based on {candidate.transactionIds.length} transactions, avg.{' '}
                      {candidate.avgDaysBetween?.toFixed(0)} days apart)
                    </p>
                    <div
                      className={`mt-2 flex space-x-2 ${currentStatus === 'dismissed' ? 'hidden' : ''}`}
                    >
                      {' '}
                      {/* Hide buttons if dismissed */}
                      {currentStatus === 'confirmed' ? (
                        <Button
                          variant='outline'
                          size='sm'
                          onClick={() => handleSetPreference(candidate.id, null)}
                        >
                          Undo Confirm
                        </Button>
                      ) : (
                        ...
                        <>
                          <Button
                            ...
                            onClick={() => handleSetPreference(candidate.id, 'confirmed')}
                          >
                            Confirm
                          </Button>
                          <Button
                            ...
                            onClick={() => handleSetPreference(candidate.id, 'dismissed')}
                          >
                            Dismiss
                          </Button>
                        </>

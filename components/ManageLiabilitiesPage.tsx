...
const LIABILITY_TYPES = ['Credit Card', 'Mortgage', 'Student Loan', 'Personal Loan', 'Auto Loan', 'Medical Debt', 'Other'] as const;
...
export default function ManageLiabilitiesPage() {
  ...
  const handleSaveLiability = async () => {
    if (!formData.name.trim()) {
      toast({ title: "Validation Error", description: "Liability name cannot be empty.", variant: "destructive" });
      return;
    }
    const currentBalanceNum = parseFloat(formData.currentBalance);
    if (isNaN(currentBalanceNum) || currentBalanceNum < 0) {
      toast({ title: "Validation Error", description: "Current balance must be a valid positive number.", variant: "destructive" });
      return;
    }
    ...
    try {
      if (editingLiability) {
        ...
        toast({ title: "Liability Updated", description: `${liabilityPayload.name} has been updated successfully.` });
      } else {
        await addLiability(liabilityPayload);
        toast({ title: "Liability Added", description: `${liabilityPayload.name} has been added successfully.` });
      }
      closeDialog();
    } catch (error) {
      logger.error("Error saving liability:", error);
      toast({ title: "Save Error", description: "Failed to save liability. Please try again.", variant: "destructive" });
    }
  };
  ...
  const handleDeleteLiability = async (liabilityId: string) => {
    ...
    try {
      ...
      toast({ title: "Liability Deleted", description: `${liabilityToDelete.name} has been deleted.` });
      setLiabilityToDelete(null);
    } catch (error) {
      logger.error("Error deleting liability:", error);
      toast({ title: "Delete Error", description: "Failed to delete liability. Please try again.", variant: "destructive" });
      setLiabilityToDelete(null);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 md:p-6">
      <Card>
        ...
        <CardContent>
          ...
          {!loading && liabilities.length === 0 && (
            <p className="text-center text-gray-500 py-8">No liabilities added yet. Click "Add New Liability" to get started.</p>
          )}
          {!loading && liabilities.length > 0 && (
                ...
                <TableBody>
                  {liabilities.map((liability) => (
                    <TableRow key={liability.id}>
                      ...
                      <TableCell className="text-right">{formatCurrency(liability.currentBalance)}</TableCell>
                      <TableCell>{formatDate(liability.lastUpdated)}</TableCell>
                      <TableCell className="text-center space-x-2">
                        <Button variant="outline" size="icon" onClick={() => openEditDialog(liability)}>
                          <Edit className="h-4 w-4" />
                          <span className="sr-only">Edit Liability</span>
                        </Button>
                        <DialogTrigger asChild>
                           <Button variant="outline" size="icon" className="text-red-600 hover:text-red-700" onClick={() => setLiabilityToDelete(liability)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => { if (!open) closeDialog(); else setIsDialogOpen(true); }}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingLiability ? 'Update the details of your liability.' : 'Fill in the details of your new liability.'}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="name" className="text-right">Name</Label>
              <Input id="name" name="name" value={formData.name} onChange={handleInputChange} className="col-span-3" placeholder="e.g., Credit Card Debt" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="currentBalance" name="currentBalance" type="number" value={formData.currentBalance} onChange={handleInputChange} className="col-span-3" placeholder="e.g., 1500" />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!liabilityToDelete} onOpenChange={(open) => { if (!open) setLiabilityToDelete(null); }}>
        <DialogContent>
            <DialogHeader>
                <DialogTitle>Confirm Deletion</DialogTitle>
                <DialogDescription>
                    Are you sure you want to delete the liability "{liabilityToDelete?.name}"? This action cannot be undone.
                </DialogDescription>
            </DialogHeader>
            <DialogFooter>
                <Button variant="outline" onClick={() => setLiabilityToDelete(null)}>Cancel</Button>
                <Button variant="destructive" onClick={() => liabilityToDelete && handleDeleteLiability(liabilityToDelete.id)}>Delete</Button>
            </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>

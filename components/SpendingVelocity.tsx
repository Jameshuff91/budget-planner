...
export default function SpendingVelocity({ selectedYear }: SpendingVelocityProps) {
  ...
  const velocityData = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const currentSpending = i === currentMonth 
        ? cumulativeSpending[currentDay - 1] || 0
        : totalSpending;
      
      const projectedTotal = i === currentMonth 
        ? (currentSpending / daysElapsed) * daysInMonth
        : totalSpending;
      
      const velocity = i === currentMonth && daysElapsed > 0
  ...
  const currentMonthData = velocityData.find(d => d.monthIndex === new Date().getMonth()) || velocityData[0];
  ...
  const velocityTrend = useMemo(() => {
    ...
    const velocityChange = ((currentVelocity - lastMonthData.avgDailySpending) / lastMonthData.avgDailySpending) * 100;
  ...
  const getVelocityIcon = (trend: string) => {
    switch (trend) {
      ...
      case 'insufficient': return <AlertTriangle className="h-5 w-5 text-yellow-500" />;
    ...
    <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  {velocityTrend === 'increasing' ? 'Spending faster' :
                   velocityTrend === 'decreasing' ? 'Spending slower' :
                   velocityTrend === 'insufficient' ? 'Not enough data' :
                   'Stable spending'}
                </span>
              </div>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-1">
              ...
              {currentMonthData && currentMonthData.projectedTotal > currentMonthData.totalSpending * 1.1 && (
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Budget remaining: {formatCurrency(
                  Math.max(0, (currentMonthData?.totalSpending || 0) - (currentMonthData?.currentSpending || 0))
                )}
              </p>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Average daily spending by month for {selectedYear}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {velocityData.slice(0, new Date().getMonth() + 1).map((month) => {
              ...
              return (
                <div key={month.monthIndex} className="flex items-center justify-between py-2 border-b last:border-0">

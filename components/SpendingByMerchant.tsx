          ...
          <ScrollArea className="h-[350px] pr-4"> {/* Adjust height as needed */}
            <div className="space-y-2">
              {merchantSpending.map((merchant, index) => (
                ...
                >
                  <div className="flex-grow mr-2 overflow-hidden"> {/* Added overflow-hidden for long names */}
                    <p className="font-semibold text-sm truncate" title={merchant.name}>{merchant.name}</p> {/* Added truncate and title */}
                    <p className="text-xs text-gray-500">{merchant.transactionCount} transaction{merchant.transactionCount === 1 ? '' : 's'}</p>
                  </div>
                  <p className="font-semibold text-sm whitespace-nowrap">{formatCurrency(merchant.value)}</p> {/* Added whitespace-nowrap */}
                </div>
              ))}
            </div>
          </ScrollArea>

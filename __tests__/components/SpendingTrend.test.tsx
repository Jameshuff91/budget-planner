...
describe('SpendingTrend Component', () => {
  ...
  test('displays no data message when spendingOverview is empty', () => {
    ...
    expect(screen.getByText('Upload bank statements to see spending trends and forecasts')).toBeInTheDocument();
    expect(screen.getByText('• Trends require at least 2 months of data')).toBeInTheDocument();
    expect(screen.getByText('• Forecasts are calculated using linear regression')).toBeInTheDocument();
  });

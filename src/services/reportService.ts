...
export class ReportService {
  ...
  public async generatePDFReport(
    ...
    try {
      logger.info('Starting PDF report generation', { options, dataPoints: reportData.transactions.length });
  ...
  private addReportHeader(pdf: jsPDF, options: ReportOptions, y: number): number {
    ...
    const title = options.reportTitle || `Financial Report - ${options.reportType.charAt(0).toUpperCase() + options.reportType.slice(1)}`;
    ...
    pdf.text(`Generated on ${format(new Date(), 'MMM dd, yyyy HH:mm')}`, pageWidth / 2, y, { align: 'center' });
  ...
  private addCategorySummary(pdf: jsPDF, reportData: ReportData, y: number): number {
    ...
    reportData.categorySpending
      ...
      .forEach(category => {
        ...
        const percentage = totalSpending > 0 ? ((category.value / totalSpending) * 100).toFixed(1) : '0.0';
   ...
   */
  private addTransactionDetails(pdf: jsPDF, reportData: ReportData, y: number, options: ReportOptions): number {
    pdf.setFontSize(16);
    pdf.setFont(undefined, 'bold');
    ...
    if (reportData.transactions.length > maxTransactions) {
      pdf.setFontSize(10);
      pdf.setTextColor(255, 0, 0);
      pdf.text(`Note: Showing first ${maxTransactions} transactions of ${reportData.transactions.length} total`, 20, y);
      pdf.setTextColor(0, 0, 0);
      y += 8;
    }
  ...
  private addTaxReport(pdf: jsPDF, reportData: ReportData, y: number): number {
    ...
    const taxDeductibleExpenses = reportData.transactions.filter(
      t => t.type === 'expense' && taxCategories.some(cat => 
    ...
    pdf.setFontSize(12);
    pdf.setFont(undefined, 'normal');
    pdf.text(`Potentially tax-deductible expenses: ${taxDeductibleExpenses.length} transactions`, 20, y);
    y += 10;
  ...
  private addRecurringAnalysis(pdf: jsPDF, reportData: ReportData, y: number): number {
    ...
    pdf.setFontSize(12);
    pdf.setFont(undefined, 'normal');
    pdf.text(`Found ${reportData.potentialRecurringTransactions.length} recurring transaction patterns:`, 20, y);
    y += 15;
    ...
    reportData.potentialRecurringTransactions.slice(0, 10).forEach(recurring => {
      ...
      pdf.setFont(undefined, 'normal');
      pdf.text(`Frequency: ${recurring.frequency} (${recurring.transactions.length} occurrences)`, 30, y);
      y += 6;
      
      if (recurring.nextEstimatedDate) {
        pdf.text(`Next expected: ${format(new Date(recurring.nextEstimatedDate), 'MMM dd, yyyy')}`, 30, y);
        y += 6;
      }
  ...
  private addFooter(pdf: jsPDF): void {
    const pageCount = (pdf as any).getNumberOfPages ? (pdf as any).getNumberOfPages() : pdf.internal.pages?.length || 1;

...
// Lazy-loaded Plaid service instance
...
export class PlaidService {
  ...
  async createLinkToken(userId: string, products: string[] = ['transactions']): Promise<string> {
    ...
    return monitor.measureAsync('plaid_create_link_token', async () => {
      try {
  ...
  async exchangePublicToken(publicToken: string): Promise<string> {
    ...
    return monitor.measureAsync('plaid_exchange_token', async () => {
      try {
  ...
  async getAccounts(accessToken: string): Promise<PlaidAccount[]> {
    ...
    return monitor.measureAsync('plaid_get_accounts', async () => {
      try {
  ...
  async getTransactions(
    ...
    return monitor.measureAsync('plaid_get_transactions', async () => {
      try {
  ...
  async syncTransactions(
    ...
    return monitor.measureAsync('plaid_sync_transactions', async () => {
      try {
...
/**
 * Factory function to create Plaid service instance with lazy loading
 */
export async function createPlaidService(): Promise<PlaidService | null> {
  ...
  return monitor.measureAsync('plaid_service_init', async () => {
    ...
    plaidServiceInstance = new PlaidService({
      clientId,
      secret,
      environment,
    });
...
/**
 * Dynamic import wrapper for Plaid Link component
 */
export async function loadPlaidLink() {
  const monitor = getPerformanceMonitor();

  return monitor.measureAsync('plaid_link_load', async () => {
...
/**
 * Preload Plaid dependencies for better performance
 */
export async function preloadPlaidDependencies() {
  const monitor = getPerformanceMonitor();

  await monitor.measureAsync('plaid_dependencies_preload', async () => {
    await Promise.all([
      import('react-plaid-link'),
      // Preload the service if credentials are available
      createPlaidService(),
    ]);
  });
}

...
// Register endpoint
router.post('/register', validateRegistration, auditLog('REGISTER', 'auth'), async (req, res) => {
  try {
...
// Login endpoint
router.post('/login', validateLogin, auditLog('LOGIN', 'auth'), async (req, res) => {
  try {
    // Check validation errors
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
router.post('/refresh', auditLog('REFRESH_TOKEN', 'auth'), async (req, res) => {
  try {
...
// Logout endpoint
router.post('/logout', auditLog('LOGOUT', 'auth'), async (req, res) => {
  try {
    const { refreshToken } = req.body;
